...
class Get_contentSinoptik(Get_link):
...
class Get_contentMeteoprog(Get_contentSinoptik):
    def __init__(self, html, mainClass, mainNextDay, tabsContentInner, temperature, pClassOne, pClassTwo, pClassThree, min, max):
        super(Get_contentMeteoprog, self).__init__(html, mainClass, mainNextDay, tabsContentInner, temperature, pClassOne, pClassTwo, pClassThree, min, max)
...
sinoptikRequest = Get_request(sinoptikLink)
sinoptikResponse = sinoptikRequest.get_html()
sinoptikOutputInf = Get_contentSinoptik(sinoptikResponse, 'main loaded', "bd2", 'tabsContentInner', 'temperature', 'day-link', 'date', 'month', 'min', 'max')
sinoptikOutputInf.outputInf(sinoptikOutputInf.start_getting())

# Вивід информації з meteoprog
Meteorg = Get_link()
meteorgLink = Meteorg.poiskpersMet("weather/"+Meteorg.enter_the_inf()+"/")
meteorgRequest = Get_request(meteorgLink)
meteorgResponse = meteorgRequest.get_html()
meteorgOutputInf = Get_contentMeteoprog(meteorgResponse, "activeBg", "someDayOffWeek", 'dayoffWeek', 'dayoffMonth', 'from', 'to', "asd", "asds", "asdfg")
meteorgOutputInf.outputInf(meteorgOutputInf.start_getting())
